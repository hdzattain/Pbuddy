...
| **Atlas Viewer** | 图鉴查看：多级导航图形化图鉴浏览系统 |
| **Fatigue System** | 疲劳系统：活动积累疲劳，休息恢复 |
| **Startup Language Selection** | 启动语言选择：首次启动时选择中文或英文，全局生效 |
...
A dedicated multi-level navigation system for browsing your pet's travel atlas. Access it through the Atlas View button or press `A`.

Navigation flow:
1. Language Selection - Choose English or Chinese
2. Pet Selection - Pick which pet's atlas to view
3. Atlas Detail - View graphical progress for all travel stages

The detail page displays:
- Progress bars for each of the 5 travel stages with unlock status
- Polar expedition achievement section
- Breakthrough material collection overview
- Visited locations list for each stage

**中文：**

专属的多级导航图鉴查看系统，通过图鉴按钮或按 `A` 键访问。

导航流程：
1. 语言选择 - 选择英文或中文界面
2. 宠物选择 - 选择要查看图鉴的宠物
3. 图鉴详情 - 查看所有旅行阶段的图形化进度
...
When launching Terminal Buddy for the first time, a language selection screen appears before the main interface.

启动 Terminal Buddy 时，主界面加载前会首先显示语言选择界面。

| Option | Description |
|--------|-------------|
| 简体中文 | All UI elements display in Chinese |
| English | All UI elements display in English |

After selection, all interface elements — buttons, status labels, species names, atlas view, and messages — will display in the chosen language. You can switch languages at any time using the `L` key.

选择语言后，所有界面元素（按钮、状态标签、物种名称、图鉴查看、消息）均以所选语言显示。随时可按 `L` 键切换语言。
